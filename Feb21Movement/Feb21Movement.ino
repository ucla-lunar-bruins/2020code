...
void moveTimeDirection(double ms, double mspeed, int direction)
{
  switch (direction) {
    case FORWARD:
...
void moveCmDirection(double cm, double mspeed, int direction=FORWARD) {
 Serial.println(MAX_MSPCM);
 Serial.println(cm);
 Serial.println(cm*MAX_MSPCM);
 Serial.println("Moving...");
 moveTimeDirection(cm * MAX_MSPCM, mspeed, direction);
...
void setup() {
  Serial.begin(9600);
  setupMovement();
  moveCmDirection(20, 255, FORWARD);
}
